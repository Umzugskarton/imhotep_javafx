...
public class UserTableView implements IUserTableView {
  ...
  public UserTableView(INavigateableView parentView, ChatView chatView, CommonLobby lobby,
      LobbyControlView controlView, EventBus eventBus, Connection connection, User user) {
  ...
  public void initOwnView() {
    if (this.myParent == null) {
      this.myParent = GenerateFXMLView.getINSTANCE()
          .loadView("/ui/fxml/app/lobby/userTable/UserTableView.fxml", this, eventBus);
    }
  ...
  public void initLobbyInfo() {
    ...
    userTableView.setItems(presenter.getLobby().getObservableUsers());
    this.controlView.getUserSizeLabel().setText(
        "Players:" + String.valueOf(presenter.getLobby().getObservableUsers().size()) + " / "
            + this.presenter.getLobby().getSize());
  }
