...
public class LobbyTableView implements ILobbyTableView {
  ...
  public LobbyTableView(INavigateableView parentView, EventBus eventBus, Connection connection,
      User user) {
  ...
  public void initOwnView() {
    if (this.myParent == null) {
      this.myParent = GenerateFXMLView.getINSTANCE()
          .loadView("/ui/fxml/app/main/lobbyList/LobbyListView.fxml", this, eventBus);

      this.tableColumnId
          .setCellValueFactory(new PropertyValueFactory<LobbyTableData, String>("lobbyIdString"));
      this.tableColumnName
          .setCellValueFactory(new PropertyValueFactory<LobbyTableData, String>("name"));
      this.tableColumnBelegung
          .setCellValueFactory(new PropertyValueFactory<LobbyTableData, String>("belegung"));
      ...
      tableColumnJoinButton
          .setCellFactory(param -> new TableCell<LobbyTableData, LobbyTableData>() {
            ...
            protected void updateItem(LobbyTableData data, boolean empty) {
              ...
              joinButton.setOnAction(event -> eventBus
                  .post(new ShowJoinLobbyDialogEvent(data, ViewIdentifier.MAIN_VIEW)));
            }
          });
